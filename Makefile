...
.PHONY: lint
lint: lint-go lint-brand-colour lint-breakpoints lint-compose lint-desktop lint-editions lint-images lint-imports lint-ios-graphql lint-keymap lint-release lint-routes lint-workflow-vars lint-tokens lint-web ## All linters
...
.PHONY: lint-brand-colour
lint-brand-colour: ## Enforce that the retired indigo accent does not come back as the brand colour
	@bash scripts/lint-brand-colour.sh
...
.PHONY: lint-workflow-vars
lint-workflow-vars: ## Enforce that a workflow step cannot read an env var nothing gives it
	@bash scripts/lint-workflow-vars.sh
