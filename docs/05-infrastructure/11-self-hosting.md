...
| `POLARIS_ALLOWED_ORIGINS` | empty | Extra cross-origin callers, comma separated. **Empty is correct for almost every install**: the web client is served from the same origin and needs no CORS at all, and the packaged desktop app's own scheme is allowed unconditionally. Every entry here is echoed back with `Access-Control-Allow-Credentials`, so anything you list can act as your users with their own cookies. It is a list of origins you control. It does **not** govern the MCP surface: `/mcp`, `/oauth/register`, `/oauth/token`, `/oauth/revoke` and the two `oauth-` well-known documents are bearer-only and answer any origin with a non-credentialed `Access-Control-Allow-Origin: *`, which is what lets a browser-hosted MCP client connect without being listed here |
...
app does **not** use that client for the authorize request: packaged Electron cannot host
Google Identity Services, so it opens the system browser with the **iOS** OAuth client
(same client id the iOS app hardcodes) and returns through that client's reverse-DNS
scheme. Production already lists that iOS client in `POLARIS_GOOGLE_CLIENT_IDS` — if iOS
Google sign-in works, desktop inherits the audience with no Google Cloud Console step.
Self-hosts that only configured the Web client must also add the iOS client id to
`POLARIS_GOOGLE_CLIENT_IDS` (or mint a Desktop client and point the shell at it).
