...
describe('cursor MCP install', () => {
  ...
  it('encodes a url-only config so Cursor opens OAuth against this replica', () => {
    ...
    expect(href.startsWith('cursor://anysphere.cursor-deeplink/mcp/install?name=polaris&config=')).toBe(
      true,
    );
    const encoded = href.split('config=')[1] ?? '';
    expect(JSON.parse(atob(encoded))).toEqual(cursorMcpInstallConfig('https://polaris.peixotolabs.com'));
  });
