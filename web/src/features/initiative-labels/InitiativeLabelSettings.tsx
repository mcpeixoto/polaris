...
import { archiveInitiativeLabel, createInitiativeLabel, updateInitiativeLabel } from './mutations';
...
export function InitiativeLabelSettings() {
    ...
    (store: Store) => {
      ...
      for (const label of store.initiativeLabels.values()) {
        ...
        rows.push({
          ...
          uses: store.initiativeIdsWithInitiativeLabel(label.id).size,
        });
      }
      return rows;
    },
  ...
  return (
    <div className={styles.screen}>
      <header className={styles.header}>
        <h1 className={styles.title}>Initiative labels</h1>
      </header>
      ...
      <div className={styles.body}>
        ...
        <section className={styles.section}>
          <p className={styles.sectionHint}>
            Workspace taxonomy for initiatives — separate from issue and project labels. An
            initiative can carry at most one label from each group.
          </p>

          <CreateLabel
            groups={grouped.roots.filter((row) => row.isGroup)}
            onCreate={(input) => run(createInitiativeLabel(engine, input))}
          />

          {grouped.roots.length === 0 ? (
            <EmptyState
              title="No initiative labels yet"
              description="Label initiatives by product line, region, or planning period — then scan the list by them."
            />
          ) : (
            <ul className={styles.tree}>
              {grouped.roots.map((row) => (
                <li key={row.id}>
                  <LabelRow
                    row={row}
                    onEdit={(fields) => run(updateInitiativeLabel(engine, row.id, fields))}
                    onArchive={() => run(archiveInitiativeLabel(engine, row.id))}
                  />
                  {!row.isGroup ? null : (
                      ...
                      {(grouped.children.get(row.id) ?? []).map((child) => (
                        <li key={child.id}>
                          <LabelRow
                            row={child}
                            onEdit={(fields) =>
                              run(updateInitiativeLabel(engine, child.id, fields))
                            }
                            onArchive={() => run(archiveInitiativeLabel(engine, child.id))}
                            onUngroup={() =>
                              run(updateInitiativeLabel(engine, child.id, { parentId: null }))
                            }
                          />
                        </li>
                      ))}
  ...
  return (
    <div className={styles.row}>
      ...
      <span className={styles.uses}>
        {row.isGroup ? '' : `${row.uses} ${row.uses === 1 ? 'initiative' : 'initiatives'}`}
      </span>
      <span>
