...
export function useView({
  ...
  const writeParams = useCallback(
    (mutate: (next: URLSearchParams) => void) => {
      // The bar as it stands, not as this render saw it — the same rule, and for the same
      // reason, as the arrival effect below. `navigate` replaces the entry synchronously,
      // so the location already carries anything written since the last commit, while
      // `params` is a snapshot that is only refreshed when React commits again. Two writes
      // between commits are ordinary: a chord fires, and the store write it triggers
      // re-renders the tree before the router's state update has landed.
  ...
  const setDisplay = useCallback(
    (patch: DisplayPatch) => {
      // Resolved the same way `display` is, but from the bar as it stands rather than as
