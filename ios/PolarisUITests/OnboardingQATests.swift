...
final class OnboardingQATests: XCTestCase {
    ...
    func testDerivedKeysSatisfyTheServerPatternsForAwkwardNames() {
        ...
        // is what saves "X".
        // Deliberately not "Peixoto Labs": it derives exactly "peixoto-labs", which is this
        // field's placeholder, and an empty XCUIElement reports its placeholder as its value.
        // The helper cannot tell the two apart, so that one name makes a correct derivation
        // look like no derivation at all.
        for workspaceName in ["Acme Corp", "Café Ltd", "3M Design", "42", "X", "  Spaced  Out  "] {
            name.tap()
            clear(name)
        ...
        // Deliberately not "Engineering": it derives exactly "ENG", which is this field's
        // placeholder, and an empty XCUIElement reports its placeholder as its value — so the
        // one team name that derives perfectly would look like no derivation at all.
        for team in ["Platform", "3M Design", "Café"] {
            teamName.tap()
            clear(teamName)
